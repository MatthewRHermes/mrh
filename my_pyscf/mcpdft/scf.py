...
def get_heff_cas (mc, mo_coeff, ci):
    ...
    veff1, veff2 = mc.get_pdft_veff (mo=mo_coeff, ci=ci0, incl_coul=False, paaa_only=True)
    dm_core = 2 * mo_core @ mo_core.conj ().T
    vj_core = self._scf.get_j (dm=dm_core)
    h1 = mo_cas @ (self.get_hcore () + veff1 + vj_core) @ mo_cas.conj ().T
...
def casci(mc, mo_coeff, ci0=None, eris=None, verbose=None, envs=None):
    ...
    for it in range (mc.max_cycle_fp):
        h0, h1, h2 = mc.get_heff_cas (mc, mo_coeff, ci1)
