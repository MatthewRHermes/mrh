...
class _ERIS(object):
    ...
    def _accumulate_ppaa_incore (self, ot, rho, Pi, ao, weight, rho_c, rho_a, vPi,
            non0tab, shls_slice, ao_loc):
        ...
        nocc = ncore + ncas
        nderiv = vPi.shape[0]
        mo_cas = _grid_ao2mo (self.mol, ao[:nderiv], mo_coeff[:,ncore:nocc], non0tab)
        if self.aaaa_only:
    ...
    def _accumulate_j_pc (self, ot, rho, Pi, ao, weight, rho_c, rho_a, vPi,
        ...
        if self.verbose > logger.DEBUG:
            mo = _square_ao (_grid_ao2mo (self.mol, ao[:nderiv], mo_coeff, non0tab))
            mo_core = mo[:,:,:ncore]
...
def kernel (ot, oneCDMs_amo, twoCDM_amo, mo_coeff, ncore, ncas, max_memory=2000,
        hermi=1, paaa_only=False, aaaa_only=False):
