...
class ContractHamCI_CHS (ContractHamCI_SHS):
    def __init__(self, las, ints, nlas, lroots, h0, h1, h2, si_ket,
                 ...
                 dtype=np.float64):
        si_bra, si_ket = fake_si_bra (lroots, si_ket, mask_bra_space)
...
def fake_si_bra (lroots, si_ket, mask_bra_space):
    ...
    assert (np.all (lroots[0]==lroots[1]))
    assert (si_ket.shape[1] == 1)
    nprods = lroots.prod (0)
    offs = np.cumsum (nprods) - nprods
    lroots_bra = lroots[:,mask_bra_space]
    ncols = lroots_bra[0].sum ()
    si_ket = np.concatenate ([si_ket,]*ncols, axis=1)
    si_bra = np.zeros_like (si_ket)
    p = 0
    for iroot in mask_bra_space:
        i = offs[iroot]
        lr = lroots[0,iroot]
        idx = i + np.arange (lr) * (lr + 1)
        q = p + lr
        si_bra[idx,p:q] = np.eye (lr)
        p = q
    assert (p==ncols)
