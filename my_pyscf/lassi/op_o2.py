...
class LRRDMint (op_o1.LSTDMint2)
    __doc__ = op_o1.LSTDMint2.__doc__ + '''
    ...
    def get_frag_transposed_sivec (self, iroot, *inv):
        '''A single-rootspace slice of the SI vectors, transposed so that uninvolved fragments
        are slower-moving
        ...
        '''
        axesorder = [i for i in range (self.nfrags) if not (i in inv)] + inv + [self.nfrags,]
        sivec = self.get_single_rootspace_sivec (root).transpose (*axesorder)
    ...
    def get_fdms (self, rbra, rket, *inv):
        ...
        axesorder = sum ([[i, i+len(inv)] for i in range (inv)], []) + [2*len(inv),]
