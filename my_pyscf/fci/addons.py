...
def add_doubly_occupied_orbital (ci0, norb, nelec, _cre_op_in_front=False):
    ...
    if _cre_op_in_front and (nelecb%2): ci1 *= -1
    if single_vector: 
        ci1 = ci1[0]
    elif is_list:
        ci1 = list (ci1)
...
def add_singly_occupied_bottom_orbital (ci0, norb, nelec, spin, _cre_op_in_front=False):
    '''Add a singly-occupied orbital of the given spin in the first position'''
    is_list = isinstance (ci0, list)
    is_tuple = isinstance (ci0, tuple)
    ci0 = add_doubly_occupied_orbital (ci0, norb, nelec, _cre_op_in_front=False)
    neleca, nelecb = _unpack_nelec (nelec)
    if _cre_op_in_front:
        nperms = neleca
        if spin: nperms += nelecb
        sgn = (1,-1)[nperms%2]
    else:
        sgn = 1
    norb = norb + 1
    ...
    for i in range (len (ci0)):
        ci1.append (sgn*des_op (ci0[i], norb, (neleca,nelecb), 0))
    if single_vector: 
        ci1 = ci1[0]
    elif is_list:
        ci1 = list (ci1)
