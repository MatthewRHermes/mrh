    ...
    if gpu_run:
        def init_eri_gpu_v4 (mo, casscf, with_df):
         ...
         def init_eri_cpu (mo, casscf, with_df):
             ...
             return mf, j_pc_cpu, k_pc_cpu, ppaa_cpu, papa_cpu

class KnownValues (unittest.TestCase):

    def test_implementations (self):
        mf_gpu, j_pc_gpu, k_pc_gpu, ppaa_gpu, papa_gpu = _run_mod (gpu_run=True)
        with self.subTest ('GPU accelerated calculation converged'):
            self.assertTrue (mf.converged)
        mf_cpu, j_pc_cpu, k_pc_cpu, ppaa_cpu, papa_cpu = _run_mod (gpu_run=False)
        with self.subTest ('CPU-only calculation converged'):
            self.assertTrue (mf_cpu.converged)
        with self.subTest ('JKs'):
            self.assertAlmostEqual (j_pc_cpu.e_tot, j_pc_gpu.e_tot, 7)
            self.assertAlmostEqual (k_pc_cpu.e_tot, k_pc_gpu.e_tot, 7)
        with self.subTest ('Intermediates'):
            self.assertAlmostEqual (ppaa_cpu.e_tot, ppaa_gpu.e_tot, 7)
            self.assertAlmostEqual (papa_cpu.e_tot, papa_gpu.e_tot, 7)
