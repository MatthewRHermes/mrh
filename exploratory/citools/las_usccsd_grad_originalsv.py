...
def get_grad_h1t2(a_idxs, i_idxs, len_t1a_arrays, las_rdm2, h):
    ...
    for a,i in zip(t2a_arrays, t2i_arrays):
        ...
        for p in range(nso):
            ...
            #h1t2 += (2*h1_mat[p,u]*rdm2[p,x,v,y])-(2*h1_mat[p,v]*rdm2[p,x,u,y])+(2*h1_mat[p,y]*rdm2[p,u,x,v])-(2*h1_mat[p,x]*rdm2[p,u,y,v])
            #print (u,v,x,y,p ,"--> ",h1t2)
            #h1t2 += h1_mat[p,u]*rdm2[p,x,v,y] - h1_mat[p,v]*rdm2[p,x,u,y] - h1_mat[p,x]*rdm2[p,u,y,v] + h1_mat[p,y]*rdm2[p,u,x,v] - h1_mat[x,p]*rdm2[u,p,v,y] + h1_mat[y,p]*rdm2[u,p,v,x] + h1_mat[u,p]*rdm2[x,p,y,v] - h1_mat[v,p]*rdm2[x,p,y,u]
        h1_t2.append(h1t2)
        print (h1t2)
...
def get_grad_h2t1(a_idxs, i_idxs, las_rdm2, h):
    ...
    for u,x in zip(t1a_arrays,t1i_arrays):
        ...
        for p in range(nso):
            for q in range(nso):
                for s in range(nso):
                    h2t1 += h2_mat[u,p,s,q]*rdm2[q,p,s,x] + h2_mat[s,p,u,q]*rdm2[q,p,x,s] - h2_mat[p,x,s,q]*rdm2[q,u,s,p] - h2_mat[q,p,s,x]*rdm2[u,p,s,q] - h2_mat[p,u,s,q]*rdm2[q,x,s,p] - h2_mat[q,p,s,u]*rdm2[x,p,s,q] + h2_mat[x,p,s,q]*rdm2[q,p,s,u] + h2_mat[s,p,x,q]*rdm2[q,p,u,s]
                    #print ("ft, st = ", ft,st)
...
def get_grad_h2t2(a_idxs, i_idxs, len_t1a_arrays, las_rdm2, las_rdm3, h):
    ...
    for a,i in zip(t2a_arrays, t2i_arrays):
        ...
        for p in range(nso):
            for q in range(nso):
                for s in range(nso):
                    h2t2_3rdm += g[u,p,s,q]*rdm3[x,q,p,y,s,v] + g[x,p,s,q]*rdm3[u,q,p,v,s,y] + g[s,p,u,q]*rdm3[x,q,p,y,v,s] + g[s,p,x,q]*rdm3[u,q,p,v,y,s] - g[q,v,s,p]*rdm3[p,x,u,s,y,q] - g[q,p,s,v]*rdm3[p,x,u,q,y,s] - g[q,y,s,p]*rdm3[p,x,u,s,q,v] - g[q,p,s,y]*rdm3[p,x,u,q,s,v] - g[v,p,s,q]*rdm3[y,q,p,x,s,u] - g[y,p,s,q]*rdm3[v,q,p,u,s,x] - g[s,p,v,q]*rdm3[y,q,p,x,u,s] - g[s,p,y,q]*rdm3[v,q,p,u,x,s] + g[q,u,s,p]*rdm3[p,y,v,s,x,q] + g[q,p,s,u]*rdm3[p,y,v,q,x,s] + g[q,x,s,p]*rdm3[p,y,v,s,q,u] + g[q,p,s,x]*rdm3[p,y,v,q,s,u]
    ...
    for a,i in zip(t2a_arrays, t2i_arrays):
        ...
        for p in range(nso):
            for q in range(nso):
                h2t2_2rdm += g[u,p,x,q]*rdm2[q,p,y,v] + g[x,p,u,q]*rdm2[q,p,v,y] - g[p,v,q,y]*rdm2[x,u,q,p] - g[p,y,q,v]*rdm2[x,u,p,q] - g[v,p,y,q]*rdm2[q,p,x,u] - g[y,p,v,q]*rdm2[q,p,u,x] + g[p,u,q,x]*rdm2[y,v,q,p] + g[p,x,q,u]*rdm2[y,v,p,q]
