...
Attempting to take advantage of the spin0 symmetry. Still it needs a lot of work.

This version follows the PySCF direct_spin0 triangular CI-space logic, but it
still uses the full orbital-pair basis nnorb = norb*norb and normal _LinkT
because the Hamiltonian/integrals are not assumed to have tril pair symmetry.
 ...
 *
 * This mirrors the PySCF direct_spin0 pattern more closely than the beta-only
 * version.  It keeps both the beta-side spread into ci1 and the alpha-side
 * spread into the thread-private ci1buf, but the alpha-side spread uses
 * bcount_for_spread_a = MIN(blen, strk-ib).  Python must then do out + out.T.
 *
 * Important: This still uses the full orbital-pair basis nnorb = norb*norb.
 * It does not use tril orbital-pair links.
 */
 ...
 *
 *   bcount_for_spread_a = MIN(strb_blksize, strk - ib)
 *   bcount              = MIN(strb_blksize, strk + 1 - ib)
 *
...
/* Explicit spin0 half-contraction with default STRB_BLKSIZE. */
