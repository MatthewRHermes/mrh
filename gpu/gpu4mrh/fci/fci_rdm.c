...
#pragma omp for schedule(dynamic, 40)
        //TODO: make all possibilites of drv into single function on it's own.
        //TODO: split into 4 different GPU calls. 
           //FCIrdm2_a/b_t1ci
           //FCIrdm2_a/b_t1ci
           //Batched GEMV (when applicable)
           //Batched GEMM //Add higher symmetry calls
        for (strk = 0; strk < na; strk++) { //na<14000
                for (ib = 0; ib < nb; ib += BUFBASE) {  //nb<14000, BUFBASE=96
                    ...
                    _LinkT *clink_indexa, _LinkT *clink_indexb, int symm)
{
        ...
        double *buf1 = calloc(norb*bcount, sizeof(double));
                    ...
                    _LinkT *clink_indexa, _LinkT *clink_indexb, int symm)
{
        ...
        double csum;
        double *buf0 = calloc(nnorb*bcount, sizeof(double));
        double *buf1 = calloc(nnorb*bcount, sizeof(double)b;
                   ...
                   int *link_indexa, int *link_indexb)
{
        ...
        double *pci0, *pci1;
        double *ci0 = ciket;
        _LinkT *tab; //_LinkT is a struct containing the a,i, new str and sign for a particular excitation
        _LinkT *clink = malloc(sizeof(_LinkT) * nlinka * na); 
        FCIcompress_link(clink, link_indexa, norb, na, nlinka);  // compresses link_indexa (a list of list in python) to contiguous block

        NPdset0(rdm1, norb*norb); //sets rdm to zero

        for (str0 = 0; str0 < na; str0++) {
                tab = clink + str0 * nlinka; //for each alpha string 
                pci0 = ci0 + str0 * nb; //get CI ket
                for (j = 0; j < nlinka; j++) { 
                        a    = EXTRACT_CRE (tab[j]);
                        i    = EXTRACT_DES (tab[j]);
                        str1 = EXTRACT_ADDR(tab[j]);
                        sign = EXTRACT_SIGN(tab[j]); //decompress and get data
                        pci1 = ci0 + str1 * nb; //should be cibra?
                        if (a >= i) {
                                if (sign == 0) {
                                        break;
                                } else if (sign > 0) {
                                        for (k = 0; k < nb; k++) {
                                                rdm1[a*norb+i] += pci0[k]*pci1[k]; //add to corresponding rdm element
                                        }
                                } else {
                                        for (k = 0; k < nb; k++) {
        ...
        free(clink);
}  // function ends, no use of cibra at all? or nlinkb? 
