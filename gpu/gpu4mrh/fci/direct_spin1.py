...
def trans_rdm12s_o1(cibra, ciket, norb, nelec, link_index=None, reorder=True):
    ...
    if reorder: libgpu.reorder_rdm(gpu, norb)
    dm1a = numpy.empty((norb, norb))
    dm2aa = numpy.empty((norb, norb, norb, norb))
    libgpu.pull_tdm1(gpu, dm1a, norb)
    libgpu.pull_tdm2(gpu, dm2aa, norb)
    ...
    libgpu.pull_tdm1(gpu, dm1b, norb)
    libgpu.pull_tdm2(gpu, dm2bb, norb)
    ...
    libgpu.pull_tdm2(gpu, dm2ab, norb)
    ...
    libgpu.pull_tdm2(gpu, dm2ba, norb)
    dm2ba = dm2ba.transpose(3,2,1,0)
