...
xyz = '''H 0.0 0.0 0.0
         H 1.0 0.0 0.0'''
mol = gto.M (atom = xyz, basis = '6-31g', output='h2_631g.log',
    verbose=lib.logger.DEBUG)
mf = scf.RHF (mol).run ()
frag_atom_list = ((0,),(1,))

las_asyn = asyn.LASSCF (mf, (2,2), ((1,0),(0,1)), spin_sub=(2,2))
mo_loc = las_asyn.set_fragments_(frag_atom_list, mf.mo_coeff)
las_asyn.state_average_(weights=[.25,]*4,
...
las_syn = syn.LASSCF (mf, (2,2), ((1,0),(0,1)), spin_sub=(2,2))
las_syn.state_average_(weights=[.25,]*4,
                       spins=[[1,-1],[-1,1],[0,0],[0,0]],
                       smults=[[2,2],[2,2],[1,1],[1,1]],
                       charges=[[0,0],[0,0],[1,-1],[-1,1]])
